...
public class Trainer implements Battleable, Iterable<Pokemon> {
    ...
    public static void initTrainers() {
        ...
        else if (Settings.game == Game.HEARTGOLD || Settings.game == Game.SOULSILVER)
        ...
        try {
            ...
            while (in.ready()) {
                ...
                for (int i = 0; i < numPokes; i++) {
                    ...
                    long PID = (calcPIDUpper(t.trainerNumber, AI, level, s.getPokedexNum(), trainerClass) << 8)
                    ...
                    // moveset not specified with hold item
                        ...
                        pk = new Pokemon(s, level, ivs, Nature.getNature(PID));
                        Item item = Item.getItem(Integer.parseInt(pokeParts[4]));
                        pk.setHoldItem(item);
                    }
                    // moveset specified
                        ...
                        if (move4 != 0)
                            m.addMove(move4);
                        pk = new Pokemon(s, level, m, ivs, Nature.getNature(PID));
                    }
    ...
    private static boolean determineIfFemale(int trainerClass) {
        ...
        Integer[] femaleClassesDP = new Integer[] { 1, 3, 5, 7, 8, 10, 13, 17, 18, 21, 22, 25, 26, 30, 33, 35, 40, 43,
                45, 50, 54, 56, 61, 66, 69, 72, 74, 76, 77, 78, 80, 84, 85, 87, 89, 90, 92, 94, 96, };
        Integer[] femaleClassesPt = new Integer[] { 1, 3, 5, 7, 8, 10, 13, 17, 18, 21, 22, 25, 26, 30, 33, 35, 40, 43,
                45, 50, 54, 56, 61, 66, 69, 72, 74, 76, 77, 78, 80, 84, 85, 87, 89, 90, 92, 94, 96, 98, 99, 101 };
        // questionable: 122 (Young Couple), 121 (Double Team)
        // also have to find the female Grunt class
        Integer[] femaleClassesHGSS = new Integer[] { 3, 5, 8, 21, 25, 36, 43, 47, 56, 70, 74, 76, 77, 82, 88, 90, 92,
                94, 99, 101, 103, 105, 106, 107, 114 };
    ...
    private static int calcPIDUpper(int trainerNumber, int AI, int level, int pokedexNum, int trainerClass) {
