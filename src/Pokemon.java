public class Pokemon implements Battleable {
	...
	public Pokemon(Species s, int newLevel) {
		this(s, newLevel, Moveset.defaultMoveset(s, newLevel, Settings.game),
				new IVs(), Nature.getNature(0), true);
	}
	...
	public Pokemon(Species s, int newLevel, Moveset moves, IVs ivs,
			Nature nature) {
		this(s, newLevel, moves, ivs, nature, false);
	}

	public Pokemon(Species s, int newLevel, Moveset moves, IVs ivs,
			Nature nature, boolean wild) {
		species = s;
		level = newLevel;
		this.moves = moves;
		this.ivs = ivs;
	...
	private int calcHPWithIV(int iv) {
		...
		return calcStatNumerator(iv, species.getBaseHP(), evs.getHPEV())
				* level / 100 + level + 10;
	}

	private int calcAtkWithIV(int iv) {
		int beforeNature = calcStatNumerator(iv, species.getBaseAtk(),
				evs.getAtkEV())
				* level / 100 + 5;
	...
	private int calcDefWithIV(int iv) {
		int beforeNature = calcStatNumerator(iv, species.getBaseDef(),
				evs.getDefEV())
				* level / 100 + 5;
	...
	private int calcSpeWithIV(int iv) {
		int beforeNature = calcStatNumerator(iv, species.getBaseSpe(),
				evs.getSpeEV())
				* level / 100 + 5;
	...
	private int calcSpcAtkWithIV(int iv) {
		int beforeNature = calcStatNumerator(iv, species.getBaseSpcAtk(),
				evs.getSpcAtkEV())
				* level / 100 + 5;
	...
	private int calcSpcDefWithIV(int iv) {
		int beforeNature = calcStatNumerator(iv, species.getBaseSpcDef(),
				evs.getSpcDefEV())
				* level / 100 + 5;
	...
	public String statsPrintout(boolean boost) {
		...
		sb.append(String.format("Stats %s badge boosts:", boost ? "WITH"
				: "WITHOUT")
				+ endl);
		sb.append(String.format("  %1$6s%2$6s%3$6s%4$6s%5$6s%6$6s", "HP",
				...
				+ endl);
		if (boost) {
			sb.append(String.format("  %1$6s%2$6s%3$6s%4$6s%5$6s%6$6s",
					getHP(), getAtk(), getDef(), getSpcAtk(), getSpcDef(),
					getSpe())
					+ endl);

		} else {
			sb.append(String.format("  %1$6s%2$6s%3$6s%4$6s%5$6s%6$6s",
					getHP(), getTrueAtk(), getTrueDef(), getTrueSpcAtk(),
					getTrueSpcDef(), getTrueSpe())
					+ endl);
		}

		sb.append(String.format("IV%1$6s%2$6s%3$6s%4$6s%5$6s%6$6s",
				ivs.getHPIV(), ivs.getAtkIV(), ivs.getDefIV(),
				ivs.getSpcAtkIV(), ivs.getSpcDefIV(), ivs.getSpeIV())
				+ endl);
		sb.append(String.format("EV%1$6s%2$6s%3$6s%4$6s%5$6s%6$6s",
				evs.getHPEV(), evs.getAtkEV(), evs.getDefEV(),
				evs.getSpcAtkEV(), evs.getSpcDefEV(), evs.getSpeEV())
				+ endl);
	...
	public void battle(Pokemon p, BattleOptions options, int participants) {
		...
		p.gainStatExp(this.getSpecies(), participants);
		p.gainExp(this.expGiven(participants));
	}
	...
	// TODO: don't recalculate stats if the food doesn't do anything (evs.eatX
	// should return bool?)
	public void eatHPUp() {
		evs.eatHPUp();
		calculateStats();
	...
	public String statRanges(boolean isBoosted) {
		...
		if (isBoosted) {
			for (int i = 0; i < 32; i++) {
				...
				possibleAtks[i] = atkBadge ? 11 * calcAtkWithIV(i) / 10
						: calcAtkWithIV(i);
				possibleDefs[i] = defBadge ? 11 * calcDefWithIV(i) / 10
						: calcDefWithIV(i);
				possibleSpcAtks[i] = spcBadge ? 11 * calcSpcAtkWithIV(i) / 10
						: calcSpcAtkWithIV(i);
				possibleSpcAtks[i] = spcBadge ? 11 * calcSpcDefWithIV(i) / 10
						: calcSpcDefWithIV(i);
				possibleSpes[i] = speBadge ? 11 * calcSpeWithIV(i) / 10
						: calcSpeWithIV(i);
			}
		} else {
			for (int i = 0; i < 32; i++) {
		...
		sb.append(String.format("%1$-6s",
				natureStatBoostReversed(NatureStat.Atk, isBoosted)));
		for (int i = 0; i < 16; i++) {
		...
		sb.append(Constants.endl);
		sb.append(String.format("%1$-6s",
				natureStatBoostReversed(NatureStat.Def, isBoosted)));
		for (int i = 0; i < 16; i++) {
		...
		sb.append(Constants.endl);
		sb.append(String.format("%1$-6s",
				natureStatBoostReversed(NatureStat.SpA, isBoosted)));
		for (int i = 0; i < 16; i++) {
		...
		sb.append(Constants.endl);
		sb.append(String.format("%1$-6s",
				natureStatBoostReversed(NatureStat.SpD, isBoosted)));
		for (int i = 0; i < 16; i++) {
		...
		sb.append(Constants.endl);
		sb.append(String.format("%1$-6s",
				natureStatBoostReversed(NatureStat.Spe, isBoosted)));
		for (int i = 0; i < 16; i++) {
