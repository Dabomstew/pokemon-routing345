...
public abstract class DamageCalculator {
	...
	protected abstract int damage(Move attack, Pokemon attacker,
			Pokemon defender, StatModifier atkMod, StatModifier defMod,
			int rangeNum, boolean crit, double basePowerMultiplier);

	public int minDamage(Move attack, Pokemon attacker, Pokemon defender,
			StatModifier atkMod, StatModifier defMod, int basePowerMultiplier) {
		return damage(attack, attacker, defender, atkMod, defMod, MIN_RANGE,
				false, basePowerMultiplier);
	}

	public int maxDamage(Move attack, Pokemon attacker, Pokemon defender,
			StatModifier atkMod, StatModifier defMod, int basePowerMultiplier) {
		return damage(attack, attacker, defender, atkMod, defMod, MAX_RANGE,
				false, basePowerMultiplier);
	}

	public int minCritDamage(Move attack, Pokemon attacker, Pokemon defender,
			StatModifier atkMod, StatModifier defMod, int basePowerMultiplier) {
		return damage(attack, attacker, defender, atkMod, defMod, MIN_RANGE,
				true, basePowerMultiplier);
	}

	public int maxCritDamage(Move attack, Pokemon attacker, Pokemon defender,
			StatModifier atkMod, StatModifier defMod, int basePowerMultiplier) {
		return damage(attack, attacker, defender, atkMod, defMod, MAX_RANGE,
				true, basePowerMultiplier);
	}
	...
	public String shortSummary(Pokemon p1, Pokemon p2, BattleOptions options) {
		StringBuilder sb = new StringBuilder();
		String endl = Constants.endl;
	...
	private String pokeStatMods(Pokemon p, StatModifier sm) {
		...
		if (sm.hasMods()) {
			sb.append(String.format("(%s) %s -> (%s) ", p.statsStr(),
					sm.summary(), sm.modStatsStr(p)));
		} else {
			sb.append(String.format("(%s) ", p.statsStr()));
		}
		sb.append("[" + p.getSpecies().getType1() + "/"
				+ p.getSpecies().getType2() + "] ");
	...
	private String summary_help(Pokemon p1, Pokemon p2, StatModifier mod1,
			StatModifier mod2) {
		StringBuilder sb = new StringBuilder();
		String endl = Constants.endl;
		...
		for (Move m : p1.getMoveset()) {
			...
			} else if (m.getType() == Type.WATER && m.getPower() > 0
					&& p1.getAbility().equalsIgnoreCase("TORRENT")) {
			...
			} else if (m.getType() == Type.FIRE && m.getPower() > 0
					&& p1.getAbility().equalsIgnoreCase("BLAZE")) {
				printMoveDamage(sb, m, p1, p2, mod1, mod2, endl, enemyHP, 1);
				Move m2 = new Move(m, "(w/ Blaze)", m.getPower() * 3 / 2);
				printMoveDamage(sb, m2, p1, p2, mod1, mod2, endl, enemyHP, 1);
			} else if (m.getType() == Type.GRASS && m.getPower() > 0
					&& p1.getAbility().equalsIgnoreCase("OVERGROW")) {
				printMoveDamage(sb, m, p1, p2, mod1, mod2, endl, enemyHP, 1);
				Move m2 = new Move(m, "(w/ Overgrow)", m.getPower() * 3 / 2);
				printMoveDamage(sb, m2, p1, p2, mod1, mod2, endl, enemyHP, 1);
			} else if (m.getType() == Type.BUG && m.getPower() > 0
					&& p1.getAbility().equalsIgnoreCase("SWARM")) {
				printMoveDamage(sb, m, p1, p2, mod1, mod2, endl, enemyHP, 1);
